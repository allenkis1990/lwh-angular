...
define(['./common/stateMapper', './common/state-config', 'baseStateMapper', 'uiRouterExtras', 'prometheus/modules/interceptor', 'angular', 'oclazyload', 'angularSanitize', 'states/home-state', './common/hbCommon', 'restangular'], function (stateMapper, stateConfig, baseStateMapper) {
    'use strict';

    stateMapper = stateConfig.merge(stateMapper, baseStateMapper, appName);

    var app = angular.module('app', ['oc.lazyLoad', 'ct.ui.router.extras', 'HB_interceptor', 'app.states.home', 'ui.router', 'ngSanitize', 'hbCommon', 'restangular']);
    app.constant('futureStates', stateMapper.futureStates);
    app.constant('modules', stateMapper.modules);

    app.config(function ($interpolateProvider, $stateProvider, $urlRouterProvider, $ocLazyLoadProvider, $futureStateProvider, HBInterceptorProvider, $httpProvider, /**restAngular配置项目的一个服务*/RestangularProvider, futureStates, modules) {
