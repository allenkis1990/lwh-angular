    ...
    var UNDEF                  = "undefined",
        ...
        ua                     = function () {
            ...
            if ( uapltform != null && uapltform.length != 0 ) {
                ...
                //comment By YangHongXiang 在IE8下会把原型链扩展方法当做一个属性输出 因此修改
                // for(var pi in pss) {
...
function p2psr_embed( type, streamType, host, streamUrl, replaceElemIdStr, widthStr, heightStr, swfVersionStr, xiSwfUrlStr, flashvarsObj, parObj, attObj, callbackFn ) {
    ...
    function getJSONP( url, cb ) {
        var fn     = 'jsonp_' + parseInt ( Math.random () * 1000000 ) + parseInt ( Math.random () * 1000000 );
    ...
    if ( type == "auto" ) {
        ...
        var f = function () {
            ...
            getJSONP ( requrl, function ( err, res ) {
                ...
                var url = fsubstreams.http_url + "/" + (streamType == "live" ? "live.M3U8" : "video.m3u8") + "?sc_tk=" + encodeURIComponent ( res.sc_tk );
                vid.setAttribute ( "src", url );
                vid.setAttribute ( "poster", fsubstreams.http_url + "/thumbnail.jpg" );
            } );
