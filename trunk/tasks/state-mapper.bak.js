...
gulp.task('stateMapper', function () {
    ...
    config.findSubDirectory('./app/center/modules')

        .forEach(function (item, index) {
            fileMap['center']['modules'][item] = {
                files: []
            };
        });
    ...
    Object.keys(item.modules)
        .forEach(function (directory, index) {
            ...
            portalSubItem.files.forEach(function (subFile, subIndex) {
                ...
                module(mapper, dirPath, stateName, deep);
                futureState(mapper, dirPath, stateName, deep);
            });
...
function module(mapper, directory, stateName, deep) {
    mapper.modules.push({
        "reconfig": true,
        "name": "app." + directory.replace(/\\/g, '.').replace(/-state$/, '') + "",
        "files": [(deep ? deep + directory + '/js/' : '') + "states/" + directory]
    });
}
