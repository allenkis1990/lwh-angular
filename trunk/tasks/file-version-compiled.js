        ...
        Object.keys(last_).forEach(function (fuck) {
            ...
            if (reg.test(fuck)) {
                ...
                Object.keys(last_).forEach(function (fuckU) {
                    ...
                    var theKey = fuckU.replace(fileReg, '$1').replace(workPlace + '/' + directory + '/js/', '').replace(workPlace + '/' + directory + '/', ''),
                        joinKey = last_[fuckU].replace(fileReg, '$1').replace(workPlace + '/' + directory + '/js/', '').replace(workPlace + '/' + directory + '/', '');

                    fileContents = fileContents.split(theKey).join(joinKey);
                });
...
function doHash(fileArray) {
    ...
    fileArray.forEach(function (file) {
        ...
        hash = crypto.createHash('md5').update(file.buffer.toString()).digest('hex').slice(0, 10);
...
function makeManifestFile() {
    return fs.writeFileSync(this.workPlace + '/manifest.json', JSON.stringify(this.manifest), { encoding: 'UTF-8' });
}
    ...
    Object.keys(this.manifest).forEach(function (srcFilePath) {
        for (var directory in directoryMap) {
            ...
            if (publicReg.test(srcFilePath)) {
                directoryMap[directory][srcFilePath] = me.manifest[srcFilePath];
            }
        }
    });
}
                ...
                if (isTxt(path.extname(file.sourcePath))) {
                    ...
                    Object.keys(directoryMap[directory]).forEach(function (manifestKey) {
                        ...
                        var ext = path.extname(manifestKey),
                            someHow = manifestKey.replace(me.workPlace + '/' + directory + '/js/', '').replace(me.workPlace + '/' + directory + '/', ''),
                            fileReg = new RegExp('(.*)\\' + ext + '$');

                        var theKey = someHow.replace(fileReg, '$1'),
                            joinKey = directoryMap[directory][manifestKey].replace(fileReg, '$1').replace(me.workPlace + '/' + directory + '/js/', '').replace(me.workPlace + '/' + directory + '/', '');

                        if (fileContents.indexOf(theKey) !== -1 && manifestKey.replace(new RegExp('^' + me.workPlace + '/'), '').indexOf(directory + '/index') === -1) {
...
module.exports = {
    doRevReplace: function (options) {
        ...
        tempFileList.forEach(function (file) {
            fileSystem.copyFileSync(file.sourcePath, file.sourcePath.replace(/^build\//, options.workPlace + '/'));
        });
...
//# sourceMappingURL=file-version-compiled.js.map