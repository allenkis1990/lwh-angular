...
define(['webuploader', 'angular'],
    function (WebUploader, angular, undefined) {
        ...
        return ['$timeout', '$parse', 'appConfig',
            function ($timeout, $parse, appConfig) {
                ...
                ////////////////////////////////////////////////////////////////////////////////
                if (!appConfig.resourceServicePath) {
                    minError("please offer " + (function () {
                            var who = '';
                            if (!appConfig.resourceServicePath) {
                                who = 'resourceServicePath';
                            }
                            return who;
                        })());
                }
                ...
                return {
                    ...
                    link: function (scope, element, attributes, ngModelCtrl) {
                        ...
                        function genServerUrl() {
                            if (appConfig.resourceServicePath.indexOf('?') === -1) {
                                appConfig.resourceServicePath += '?1=1';
                            }
                            var uploadSync = true,
                                result = [appConfig.resourceServicePath, 'uploadSync=' + uploadSync],
                                scale = getScale();
                            if (scale) {
                        ...
                        $timeout(function () {
                            ...
                            instance.on('uploadSuccess', function (file, response) {
                                if (attributes.queueContainerId) {
                                    ...
                                    imageContainer.attr('src', appConfig.resourceServicePath + uploadedNewPath);
                                    imageContainer.show();
                                }
                                $timeout(function () {
                                    // *************************
                            ...
                            instance.on('beforeFileQueued', function (file) {
                                ...
                                if (extensions.indexOf(fileExt) === -1) {
                                    alert('请上传' + defaultConfiguration.accept.extensions + '格式的图片!');
                                    return false;
                                }
                            });
