...
gulp.task('copyProjectConfig', function () {
    ...
    return gulp.src(['./app/admin/index.html']).pipe(function () {
        return through2.obj(function (file, sex, next) {
            ...
            fileContent = fileContent.replace(/(<script\s*id="project_main_config">)(.|\n|\t|\s)*?(<\/script>)/gi, '$1var projectMainConfig = ' + JSON.stringify(projectMainConfigJson) + ';$3').replace(/(<script\s*id="app_name_use_in_gulpfile">)(.|\n|\t|\s)*?(<\/script>)/gi, '$1var appName = "' + dir + '";$3');
...
gulp.task('asignIndexAppName', function () {
    ...
    return gulp.src(filter).pipe(function () {
        return through2.obj(function (file, sex, next) {
            var fileContent = file.contents.toString(),
...
//# sourceMappingURL=copy-project-config-compiled.js.map